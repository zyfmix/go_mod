...
// [go语言zap日志自定义输出](https://docs.lvrui.io/2020/03/25/go语言zap日志自定义输出/)
package logs
...
func init() {
	...
	//	options.EncoderConfig.EncodeLevel = zapcore.CapitalColorLevelEncoder
	//}
...
func SetLevel(lvl string) {
	Warn(nil, "Set logs atomicLevel "+lvl)
...
func runAtomicLevelServer(atomicLevel zap.AtomicLevel, options AtomicLevelServerOption) {
	go func() {
		...
		levelServer.HandleFunc(urlPath, func(w http.ResponseWriter, r *http.Request) {
			...
			_, logger := NewCtxLogger(r.Context(), CloneLogger("atomiclevel"), r.Header.Get(CtxTraceKey))
		...
		if err := http.ListenAndServe(options.Addr, levelServer); err != nil {
			Error(nil, "logs NewLogger levelServer ListenAndServe error:"+err.Error())
		}
	}()
}
...
func PublicIP() string {
	...
	if err != nil {
		...
		fmt.Println("My Public IP address is:", publicIP)
	}
